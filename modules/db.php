	...
	public function select($option,$data)
	{
		...
		{																																																																																																										
			...
			switch($option['lvl2'])
			{
				...
				case "by_boss":
				$id=mysqli_real_escape_string($this->cn,$data['boss']);
				echo "sera aqui?";
				$info=$this->get_data("SELECT * FROM clerk WHERE boss='$id';"); 
				break;
			...
			switch($option['lvl2'])
			{
				...
				$info=$this->get_data("SELECT * FROM canal WHERE nombre='$id';"); 
				break;
			}
			break;
			case "video":
			switch($option['lvl2'])
			{
				...
				case "one":
				$id=mysqli_real_escape_string($this->cn,$data['url']);
				$info=$this->get_data("SELECT * FROM video WHERE url='$id';"); 
				break;

				case "by_canal":
				$id=mysqli_real_escape_string($this->cn,$data['canal']);
				
				$info=$this->get_data("SELECT * FROM video WHERE canal='$id';"); 
				break;
